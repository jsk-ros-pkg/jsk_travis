...
if [ "$BUILDER" == catkin ]; then catkin build -i --summarize  --limit-status-rate 0.01 $BUILD_PKGS $CATKIN_PARALLEL_JOBS --make-args $ROS_PARALLEL_JOBS            ; fi
...
if [ "$BUILDER" == catkin ]; then
    ...
    catkin run_tests -iv --no-deps --limit-status-rate 0.01 $TEST_PKGS $CATKIN_PARALLEL_TEST_JOBS --make-args $ROS_PARALLEL_TEST_JOBS --
...
if [ "$NOT_TEST_INSTALL" != "true" ]; then
    ...
    if [ "$BUILDER" == catkin ]; then
        ...
        catkin build -i --summarize --limit-status-rate 0.01 $BUILD_PKGS $CATKIN_PARALLEL_JOBS --make-args $ROS_PARALLEL_JOBS
